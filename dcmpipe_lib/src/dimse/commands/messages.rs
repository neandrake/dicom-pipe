...
impl CommandMessage {
    ...
    pub fn c_find_req(
        ...
        msg_id: u16,
        sop_class_uid: &str,
        mut query_params: Vec<(&Tag, RawValue)>,
    ) -> Self {
        let mut tags = vec![
            (
                &AffectedSOPClassUID,
            ...
            (
                &CommandDataSetType,
                // Should be any value other than COMMAND_DATASET_TYPE_NONE
                RawValue::ushort(0x01),
            ),
        ];
        tags.append(&mut query_params);
        CommandMessage::create(ts, tags)
    }
